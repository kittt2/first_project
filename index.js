    ...
    try {
        if (alreadyExit[0]) {
            ...
            bcrypt.hash(password, 5, async (err, hash) => {
                ...
                await user.save();
                res.send({ "msg": "account is created" })
...
app.post('/login', async (req, res) => {
    ...
    alreadyExit = await userModel.findOne({ email: email });
        ...
        if (alreadyExit) {
            ...
            return res.status(501).send({ "msg": "please crreate your account first " });
        }
    } catch (error) {
        console.log(error);
...
//     try {
//         const user=await userModel.findByIdAndUpdate(id,{name:"kitt"});
//         res.send(user);
//     } catch (error) {
